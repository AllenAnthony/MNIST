...
W_conv1 = weight_variable([5,5,1,coreNum1])
#初始化b为[32],即输出大小
b_conv1 = bias_variable([coreNum1])
...
W_conv2 = weight_variable([5,5,coreNum1,coreNum2])
b_conv2 = weight_variable([coreNum2])

#h_pool2即为第二层网络输出，shape为[batch,7,7,64]
...
W_fc1 = weight_variable([7*7*coreNum2, 1024])
b_fc1 = bias_variable([1024])

#计算前需要把第2层的输出reshape成[batch, 7*7*64]的张量
h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*coreNum2])
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

#Dropout层：把1024个值中的其中一些值故意丢弃，来破坏对称性，防止过拟合，keep_prob表示数据利用率[0,1],1代表不丢弃数据
...
#开始训练模型，循环20000次，每次随机从训练集中抓取50幅图像，训练时dropout 50%，测试时不加dropout
for i in range(20000):
    batch = mnist.train.next_batch(50)
    # if i%100 == 0:
    #     #每100次输出一次日志,相当于把第101次的训练数据当成前面的测试数据
    #     train_accuracy = accuracy.eval(feed_dict={x:batch[0], y_:batch[1], keep_prob:1.0})
    #     print("step %d, training accuracy %g" % (i, train_accuracy))
    #     tmp=("step %d, training accuracy %g" % (i, train_accuracy))
    #     re.write(tmp)

    train_step.run(feed_dict={x:batch[0], y_:batch[1], keep_prob:0.5})
res = accuracy.eval(feed_dict={x:mnist.test.images, y_:mnist.test.labels, keep_prob:1.0})

re = open("record20000.txt", "a+")
re.write(__file__)
re.write("test accuracy %g \n" %res)
re.close()
